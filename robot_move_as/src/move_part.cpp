 unsigned short int RobotMoveActionServer::move_part(const robot_move_as::RobotMoveGoalConstPtr &goal,double timeout) {
            ...
            if (!hover_jspace_pose(goal->targetPart.location, box_hover_pose_)) {
                ROS_WARN("hover_jspace_pose() failed for target ");
                return errorCode;
            }
            ROS_INFO_STREAM("box_hover: " << box_hover_pose_.transpose());

            if (!hover_jspace_pose(goal->sourcePart.location, bin_hover_jspace_pose_)) {
                ROS_WARN("hover_jspace_pose() failed for source bin %d", (int) goal->sourcePart.location);
                return errorCode;
            }
            ...
            if (!cruise_jspace_pose(goal->sourcePart.location, bin_cruise_jspace_pose_)) {
                ROS_WARN("cruise_jspace_pose() failed");
                return errorCode;
            }
            //cruise pose, adjacent to bin:
