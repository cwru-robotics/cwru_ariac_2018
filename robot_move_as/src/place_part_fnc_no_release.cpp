...
//get_pickup_IK(Eigen::Affine3d affine_vacuum_gripper_pose_wrt_base_link,
//                                          Eigen::VectorXd approx_jspace_pose, bool use_wrist_far, Eigen::VectorXd &q_vec_soln) 
    /* don't need cruise pose; assume already there*/
    /*
    ...
    */
    ...
    bool use_wrist_far = part.use_wrist_far;
    if (!get_pickup_IK(affine_vacuum_pickup_pose_wrt_base_link_, bin_hover_jspace_pose_, use_wrist_far, dropoff_jspace_pose_)) {
...
unsigned short int RobotMoveActionServer::is_placeable(inventory_msgs::Part part) {
    ...
    if (!hover_jspace_pose(part.location, bin_hover_jspace_pose_)) {
        ROS_WARN("hover_jspace_pose() failed for location %d", (int) part.location);
        errorCode = robot_move_as::RobotMoveResult::WRONG_PARAMETER;
        return errorCode;
    }
    //ROS_INFO_STREAM("destination  hover pose: " << bin_hover_jspace_pose_.transpose());
